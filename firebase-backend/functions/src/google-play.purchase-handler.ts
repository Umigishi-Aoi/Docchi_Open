    ...
    try {
      ...
      try {
        ...
        console.log("Could not create or update purchase", {
          orderId: response.data.orderId,
          roductId: productData.productId});
      }
      return true;
    } catch (e) {
      console.error(e);
  ...
  async handleSubscription(
    ...
    try {
      ...
      try {
        ...
        console.log("Could not create or update purchase", {
          orderId, productId: productData.productId});
      }
      return true;
    } catch (e) {
      console.error(e);
  ...
  */
  handleServerEvent = functions.pubsub.topic(GOOGLE_PLAY_PUBSUB_BILLING_TOPIC)
      .onPublish(
          /**
        * コールバック関数
        * @param {any} this is message
        * @param {any} message is message
        */
          async function googleHandle(this: any, message: any) {
        // Define the event
        ...
        try {
          event = JSON.parse(new Buffer(message.data, "base64")
              .toString("ascii"));
        } catch (e) {
        ...
        const notificationType =
          subscriptionId ? "SUBSCRIPTION" : sku ? "NON_SUBSCRIPTION" : null;
        ...
        switch (notificationType) {
          ...
          }.bind(GooglePlayPurchaseHandler));
}
